...
class SceneExtrapolationPanel extends HTMLElement {
  ...
  async _build() {
    ...
    this.shadowRoot.innerHTML = `
      <style>
        ...
        /* Landscape clock: timeline overlays the sky on the right; the dial
           stays full-stage centered (no matching gutter column). */
        ...
        .sun-year-scrub-rail {
          ...
          top: 0;
          right: 12px;
          width: var(--scrub-rail-width, ${CLOCK_SCRUB_RAIL_PX}px);
          min-width: 0;
          overflow: hidden;
          opacity: 1;
          box-sizing: border-box;
        ...
        .sun-light-clock-face {
          position: relative;
          /* Max-width wraps only the watch face; sky bleeds past this box. */
          width: min(100%, 80vh);
          max-width: 80vh;
          aspect-ratio: 1;
          flex: 0 0 auto;
        ...
        /* Huge square centered on the face so the conic stays aligned
           while covering the page. Clipped by .page.dial-wide. */
        .clock-backdrop {
          position: absolute;
          left: 50%;
          top: 50%;
        ...
        .clock-backdrop .clock-sky-overlay {
          position: absolute;
          inset: 0;
        }
        ...
        .sun-light-clock-face.clock-face-enter .clock-float {
          animation: clock-float-enter 1500ms cubic-bezier(0.2, 0, 0, 1) both;
        }
        ...
        .clock-float {
          position: absolute;
          width: ${((CLOCK_FLOAT_R * 2) / CLOCK_VIEW) * 100}%;
          height: ${((CLOCK_FLOAT_R * 2) / CLOCK_VIEW) * 100}%;
          transform: translate(-50%, -50%);
          border-radius: 50%;
          z-index: 4;
          pointer-events: none;
  ...
  _alignYearScrubRail() {
    ...
    // Match the dial height; the rail overlays the sky on the right.
  ...
  _clockPolar(seconds, radius = CLOCK_SUN_PATH_R) {
  ...
  _applyClockSunAppearance(seconds) {
    ...
    if (handleHit) {
      handleHit.style.setProperty("--handle-deg", `${pos.deg}deg`);
    }
    const floatEl = this._clockFloatEl;
    if (floatEl) {
      // Opposite the time handle; stays inside the path (offset+r+gap < path R).
      const opposite = this._clockPolar(
  ...
  _playClockEnterAnimation(face) {
    ...
    const clearEnter = (ev) => {
      ...
      const hasEvents = face.querySelector(".clock-event-anchor");
      if (
        name &&
        name !== "clock-event-spin" &&
        !(name === "clock-overlay-spin" && !hasEvents)
      ) {
        return;
      }
      face.classList.remove("clock-face-enter");
  ...
  _paintClockSunPath(overlay, host) {
    ...
    path.setAttribute("cy", String(CLOCK_CY));
  ...
  _buildLightClock(events) {
    ...
    skyOverlay.setAttribute("class", "clock-sky-overlay");
    skyOverlay.setAttribute("viewBox", `0 0 ${CLOCK_VIEW} ${CLOCK_VIEW}`);
    this._paintClockSky(skyOverlay, events);
    backdrop.append(skyHost, horizonGlow, skyOverlay);
    ...
    floatEl.className = "clock-float";
    this._clockFloatEl = floatEl;
    ...
    const openRingAt = (ev, light) => {
      ...
      const closest = this._closestEvent(assigned, previewSeconds());
      if (closest) {
