...
- **Superseded in part:** 2026-08-30 — light-ring glow is two non-interactive dial clones (scales 2.76 + 1.38, blur 28px, opacity 0.815) behind the planet so bloom matches ring colors; the interactive rings use a 32px / 40% black drop-shadow and stack above the hour handle so the planet occludes it. Event spokes match the night sun-path dash (`#d8e0ff`, `5 4`, 1px) at 50% opacity; night path is also 50%.
