...
- **Decision:** On create/edit, a header toggle switches the light table between the stacked bands and concentric 24-hour rings. Midnight is at the bottom; hours are equal; noon is at the top. One ring per lamp (outer = first table row). The clock face is `min(100%, 100vh)` square on a full-width dial page so solar-event icons can sit in a fixed-px chrome band outside an inset dial core. Soft radial seams blend neighboring rings; hovering the rings — or having a selected ring — animates `--clock-feather` down to sharpen them. A lighter elevation glow sits behind the rings; a tight conic horizon ramp plus a farther blurred sky-color wash paint **behind** the planet and span the **full panel width** (sidebar overlaps; the dial still shifts left via the gutter). Elevation wraps the rim (angle = time, radius = elev): elev=0 is outside the planet; day extends toward the face edge; night uses a smooth elev→radius floor that clears the light rings at max sun scale (no offset-blend warp). The sun is an outlined disc with SVG fill clipped to the sunrise→sunset day wedge (black night disc underneath) plus glow/shadow; the hour handle paints above. Path stroke is a constant 1px (`vector-effect: non-scaling-stroke`): solid by day, brighter dashed by night, with 3px plain dots at solar events and dashed spokes to those dots. Size is largest (2×) at sunrise/sunset and fixed through the night; it shrinks toward 1× at zenith and grows again toward sunset. Drag the sun (or handle) to scrub with magnetic snaps to solar events and same-direction rubber-band resistance leaving them (ease back to the cursor on break); after release the time is sticky until reset (click the sun / restore control), an event is selected, the preview date changes, or the user leaves the editor. While sticky, a gold stroke on the outer tick radius runs from wall-clock now to the scrubbed time with a gold wash toward the center. Selecting a solar event pins the sun to that event. Idle without sticky/event is wall-clock now on the preview date. The linear sun chart is hidden in dial view. Solar-event icons open the event sidebar. Suggested lights stay in the legend. Remember the view in `localStorage` per HA user (`scene_extrapolation.lightView.v1.<user>`).
- **Why:** A ring makes the dusk→dawn wrap obvious. Equal clock hours match how people read “now”; solar events still land at their real times. Putting elev=0 outside the planet keeps the light rings primary while the sun can fall below the real sunrise/sunset horizon without being buried under the lamps.
- **Do not reverse without user ask.**
