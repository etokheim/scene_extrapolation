...
- **Decision:** On create/edit, a header toggle switches the light table between the stacked bands and concentric 24-hour rings. Midnight is at the bottom; hours are equal; noon is at the top. One ring per lamp (outer = first table row); rings fill to the center (no hole). The clock face is `min(100%, 100vh)` square on a full-width dial page so solar-event icons can sit in a fixed-px chrome band outside an inset dial core. Soft radial seams blend neighboring rings; hovering the rings — or having a selected ring — animates `--clock-feather` down to sharpen them. A lighter elevation glow sits behind the rings; a tight conic horizon ramp paints **behind** the planet over a solid `--card-background-color` sky wash spanning the **full panel width** (sidebar overlaps; the dial still shifts left via the gutter). Time wraps the rim on a **perfect circle** whose radius scales with today's peak elevation vs the location's annual max (summer outer, winter inner), clamped with padding so path and sun clear the planet and the dial-core edge (day solid / night dashed; 1px non-scaling). Hour ticks and labels are white at 60% opacity. The sun is an outlined disc with solid white SVG fill + white glow clipped to the sunrise→sunset day wedge (outline only below the horizon); the hour handle paints above. Size is largest (2×) at sunrise/sunset and fixed through the night; it shrinks toward 1× at zenith and grows again toward sunset. Drag the sun (or handle) to scrub with magnetic snaps to solar events (capture ~12 min / release ~29 min) and same-direction rubber-band resistance leaving them (ease back to the cursor on break); after release the time is sticky until reset (click the sun / restore control), an event is selected, the preview date changes, or the user leaves the editor. While sticky, a 1px white stroke on the outer tick radius runs from wall-clock now to the scrubbed time with a short white–gold wash toward the center. Selecting a solar event pins the sun to that event. Idle without sticky/event is wall-clock now on the preview date. The linear sun chart is hidden in dial view. Solar-event icons open the event sidebar. Suggested lights stay in the legend. Remember the view in `localStorage` per HA user (`scene_extrapolation.lightView.v1.<user>`).
