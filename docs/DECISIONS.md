...
- **Date:** 2026-08-30
- **Superseded in part:** 2026-08-30 — with Python 3.14, CI installs current HA, so `StaticPathConfig` stays a normal top-level import (lazy import tripped pylint `import-outside-toplevel`).
- **Decision:** GitHub Actions lint/tests use Python **3.14** and install the latest PyPI `homeassistant`. Workflows use current `actions/checkout` / `actions/setup-python` majors.
- **Why:** Unpinned `pip install homeassistant` on 3.11 resolves to ~2024.3, which lacks `StaticPathConfig` / `LockState`. Current HA requires Python ≥3.14.2; matching that keeps lint and unit tests on the same API surface as the sandbox.
- **Do not reverse without user ask.**
