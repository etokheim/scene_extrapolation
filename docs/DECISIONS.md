...
- **Date:** 2026-08-26
- **Superseded in part:** 2026-08-29 — on landscape devices in clock view the scrubber is a vertical rail to the right of the clock face (stable sibling of `.sun-path-body` so preview redraw cannot drop pointer capture); 30px gap from the clock; collapsing the rail animates width/gap with the sidebar dock (do not `hidden`-yank it); portrait and stacked/table view keep the horizontal strip under the date row; landscape + any open scene sidebar hides/collapses the scrubber.
...
- **Superseded in part:** 2026-08-29 — sun marker follows the drawn path (no horizon pull); scale stays 2× for all elev<0 and at 0°, then eases to 1× as daytime elevation rises; stroke width maps that day’s elev min→10px / max→0.2px (widest below the horizon) with daytime tint from `skyLookFromElevation` (night strokes stay neutral); round dashed stroke (`8 7`) at 50% opacity with continuous dash phase; hour ticks sit inside the horizon so the outward day path does not cross the tick ring; no area-fill under the linear sun curve; clock enter plays once per editor visit (reset when returning to the list; date/scene redraws skip it); adding a missing light commits undo before writing the draft.
- **Superseded in part:** 2026-08-29 — clock solar-event markers get a 0.5px dashed spoke to the horizon and upright name · time + scene labels immediately above the icon (screen-space, not radial).
