...
- **Superseded in part:** 2026-08-30 — dial ring hover/selected use `scale(1.1)`, a stronger black `drop-shadow`, and a semi-transparent white `::after` rim; sibling rings dim to 50% over a surface-colored disc (`--primary-background-color`) so they don’t mix with horizon/bloom. Soft mode keeps `--clock-feather` plus `--ring-soft-expand` (~1.35× feather) so opaque cores overlap and the surface never shows through seams; sharp mode (hover/selected host) zeros both so bands sit edge-to-edge, then the active band scales up.
