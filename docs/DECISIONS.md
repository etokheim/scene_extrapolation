...
- **Superseded in part:** 2026-08-29 — sun marker follows the drawn path (no horizon pull); scale stays 2× for all elev<0 and at 0°, then eases to 1× as daytime elevation rises; stroke width maps that day’s elev min→0.2px / max→10px with daytime tint from `skyLookFromElevation` (night strokes stay neutral secondary text); butt linecaps + continuous dash phase so dense day segments do not stack into a double spine; hour ticks sit inside the horizon so the outward day path does not cross the tick ring; no area-fill under the linear sun curve (fill edge + stroke looked like two paths); clock enter plays once per editor visit (reset when returning to the list; date/scene redraws skip it); adding a missing light commits undo before writing the draft.
