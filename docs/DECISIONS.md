...
- **Superseded:** 2026-08-29 — dial view drops the 1920px page cap. Sky/night/horizon are a face-centered disc that covers the page; only the watch face keeps `max-width: 80vh`.
- **Decision:** `.page` defaults to a centered 1024px column with 12px inline padding. Dial (clock) view adds `.dial-wide` (`max-width: none`) so the sky can span the page. Do not use `--ha-automation-editor-width`, and do not grow max-width when the drawer opens.
- **Why:** Stacked light bands stay readable at 1024px; the dial sky is a page-bleed, not a wider text column.
- **Do not reverse without user ask.**
...
- **Superseded in part:** 2026-08-29 — landscape timeline overlays the sky on the right (does not take a grid column or shrink the face). The readout and draft banner also overlay the sky. The form below the dial keeps a solid page background.
...
- **Why:** Jumping between solstices with chips is coarse; the calendar picker is precise but slow for seasonal comparison. A year strip is the missing middle. Re-inserting the scrubber on every preview cancelled the drag. Overlaying the rail keeps the face centered on the full-bleed sky.
- **Do not reverse without user ask.**
...
- **Superseded in part:** 2026-08-29 — `--light-feather` / `--clock-feather` / `--ring-expand` / `--ring-border-w` are registered with `CSS.registerProperty` on the document (shadow `@property` does not enable transitions). Transition feather on `.sun-lights` / `.sun-light-clock-rings`; expand/border on each `.clock-ring`. Ring fill uses a masked child so hover/selected `::after` rim borders are not clipped by the fill mask; selected rings keep the sharp feather (`:has(.selected)`), not only hover.
...
- **Superseded:** 2026-08-29 — Solar Dial redesign. Clock-face geometry and hover-scrub are replaced by the section below. Table view, the header toggle, ring paint, and `localStorage` light-view key are unchanged.
- **Decision:** On create/edit, a header toggle switches the light table between the stacked bands and the Solar Dial. Midnight is at the bottom; hours are equal; noon is at the top. One ring per lamp (outer = first table row). Soft radial seams blend neighboring rings; hovering the rings — or having a selected ring — animates `--clock-feather` down to sharpen them. Clicks pick a lamp by radius on the floating rings host. The lamp open in the light-edit sidebar is highlighted on its ring, legend, and stacked band. The linear sun-elevation chart is hidden in dial view. Suggested lights stay in the legend, not as empty rings. Remember the choice in `localStorage` per HA user (`scene_extrapolation.lightView.v1.<user>`), not per scene — read it before the first light paint so the toggle and the rendered view stay in sync after refresh.
...
## Solar Dial: outer time scale, circular path, floating rings

- **Date:** 2026-08-29
- **Decision:** Dial view is an Apple Solar Dial–style face, not a planet-rim with elevation-as-radius. **Watch face:** a container circle is inset in the square so event buttons fit outside it. **Sun path:** a second centered circle at 75% of that container (25% smaller); angle is time-of-day. Elevation still drives sun flare and the top-left readout — not path radius. **Time dial:** noon at top, midnight at bottom; midnight label is `24`; even hours labeled at a fixed **10px** (viewports under 871px) or **14px** (871px and up) — CSS px on HTML labels, not viewBox-scaled SVG text. Ticks every 5 minutes (major every 2h, then hour / half / quarter / 5 min), `vector-effect: non-scaling-stroke` so stroke width is the same on mobile and desktop. **Sky:** a face-centered disc of the **previewed** time’s elevation (radial wash, not a full-day snapshot) plus night/dusk wedges and one sun-tinted horizon wash, large enough to cover the page; `.page.dial-wide` clips it. Night is dark blue, not black. Only the masked sky disc (not the HA page chrome) tracks that color. Sky + solar-event chips share one page-edge mask (a blurred inset rect, not stacked color ramps) so all four sides vignette without corner overlap; the mask floors at 25% opacity (75% transparent), not fully gone. The watch face, sun, handle, rings, readout, and chrome stay unmasked. The panel surface stays `--primary-background-color`. The watch face keeps `max-width: 80vh`. Timeline, time/elevation readout, and the draft banner overlay the sky. No center glow. **Time handle:** two segments from center to the sun outline and from the far side of that outline to the watch-face rim (the line does not cross the sun disc). **Sun:** white outlined circle on the path (15% smaller than the first inset size); day fill is clipped to the day wedge (the sunrise/sunset horizon rays) so it is masked away as the disc crosses the horizon, not hidden at elev 0; a dark-blue disc sits under that fill so the night side of the circle is masked in as the day fill is eaten; once fully behind, the disc is dark blue with the white outline; size grows at most 10% toward the horizon. **Night sky:** sunset→sunrise is 50% darker toward night blue (not black). Drag the sun or the handle to scrub (pointer capture). Face-wide hover no longer moves the sun. Starting a drag closes the event sidebar and clears the pin. After release the preview time is **sticky** until an event is selected, the preview date changes, or the user leaves the editor. Idle (no sticky, no event) is wall-clock now. The readout follows the sun only. **Event icons:** today’s buttons sit **outside** the watch-face circle (same click → event sidebar) with always-visible name · time + scene labels; sunrise/sunset labels sit below the icon. Chips below the horizon use night blue, not black. A 0.5px dashed spoke runs from the button into the face to the sun path. The sun has a larger grab halo. Events without a time are omitted. **Floating rings:** the concentric full-day conics live in a circle opposite the handle (`angle + 180°`), filled to the center (no inner hole). Size/offset stay inside the path so the host does not cover icons or the sun. Scrubbing does not recolor rings. Ring hit-testing uses that host’s box; opening a lamp uses the previewed (sun) time, not the click angle on the face. **Enter:** once per editor visit; retargeted to overlay / float / events / sun sweep.
- **Why:** Elevation-as-radius fought the 24h clock. A fixed path plus solar-event wedges keep day/night honest at Nordic latitudes. Moving the rings off the face lets the sky and sun be the primary time cue without losing the full-day light preview.
- **Do not reverse without user ask.**
