...
- **Superseded in part:** 2026-08-27 — pencils are small shadowed 5px dots on a 40px hit. Hover grows one circle (the action layer) from that center; do not scale the 5px disc (it stacked on `ha-icon-button` and drifted). Click the row to edit the closest assigned scene; more-info is on the sidebar, not the name. The SVG is not a hit target (`pointer-events: none`) so the band click reaches the bar.
