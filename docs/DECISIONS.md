...
- **Date:** 2026-08-29
- **Decision:** Dial view is an Apple Solar Dial–style face, not a planet-rim with elevation-as-radius. **Watch face:** a container circle fills the square. **Sun path:** a second centered circle at 75% of that container (25% smaller); event icons sit on the path; angle is time-of-day. Elevation still drives sun flare, sky glow, and the top-left readout — not path radius. **Time dial:** noon at top, midnight at bottom; midnight label is `24`; even hours labeled; ticks and numbers sit **inside** the container stroke. **Sky:** day disc inside the path; a night wedge from sunset to sunrise; a darker dusk→dawn wedge; radial horizon rays from center through sunrise and sunset to the path. Sunrise/sunset warmth washes **up** from those rays into the day (toward noon), not from the face center. **Time handle:** thin ray from true center through the sun to the path. **Sun:** sits on the path; drag the sun or the handle to scrub (pointer capture). Face-wide hover no longer moves the sun. Starting a drag closes the event sidebar and clears the pin. After release the preview time is **sticky** until an event is selected, the preview date changes, or the user leaves the editor. Idle (no sticky, no event) is wall-clock now. The readout follows the sun only. **Event icons:** today’s rim buttons live on the sun path (same click → event sidebar) with always-visible name · time + scene labels; sunrise/sunset labels sit below the icon. No spokes. Icons sit above the sun for clicks; the sun has a larger grab halo. Events without a time are omitted. **Floating rings:** the concentric full-day conics live in a circle opposite the handle (`angle + 180°`). Size/offset stay strictly inside the path (`offset + radius + gap < path R`) so the host never covers icons or the sun. Scrubbing does not recolor rings. Ring hit-testing uses that host’s box; opening a lamp uses the previewed (sun) time, not the click angle on the face. **Enter:** once per editor visit; retargeted to overlay / float / events / sun sweep.
