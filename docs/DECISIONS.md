...
- **Decision:** On create/edit, a toolbar toggle (next to the location pin) switches the light table between the stacked bands and concentric 24-hour rings. Midnight is at the top; hours are equal; noon is at the bottom. One ring per lamp (outer = first table row). The clock face is `min(100%, 80vh)` square so it fills the column without growing taller than the viewport. A soft glow behind the rings is a blurred, scaled disc painted with the outer ring’s conic gradient (not a second full set of rings): blur 81px, scale 1.35. Soft radial seams blend neighboring rings (same idea as the table feather); hovering the rings animates `--clock-feather` down to sharpen them. Rings overlap by the feather amount so the blend sits on lamp color, not the dark card. Clicks pick a lamp by radius (masked rings still fill the square for hit-testing). The lamp open in the light-edit sidebar is highlighted on its ring, legend, and stacked band. The linear sun-elevation chart is hidden in clock view; elevation wraps the rim instead — outer ring edge is the horizon (“planet”), day path is a solid 3px stroke outside it (radial span exaggerated ~2× so noon sits farther from the rim), night path is a dashed 2px stroke over the face. A sunny icon marks the sun’s position at the current time of day on the preview date. Scale uses the location’s annual max elevation (same as the linear chart). Today draws an hour hand on the clock. Solar-event icons sit on the rim and open the same event sidebar as the chips above the sun chart; in clock view those top chips are hidden (the rim replaces them). Unassigned rim controls stay 32px tall but widen into a pill with icon + “Choose scene” (same wording as the table chips). Per-event pencil dots stay on the stacked view only. Hover maps angle → seconds (radial cursor); a legend lists names, brightness %, Add, and Remove. Suggested lights (no samples) stay in the legend, not as empty rings. Remember the choice in `localStorage` per HA user (`scene_extrapolation.lightView.v1.<user>`), not per scene — read it before the first light paint so the toggle and the rendered view stay in sync after refresh.
