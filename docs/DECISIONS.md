...
- **Decision:** On create/edit, a header toggle switches the light table between the stacked bands and concentric 24-hour rings. Midnight is at the bottom; hours are equal; noon is at the top. One ring per lamp (outer = first table row). The clock face is `min(100%, 100vh)` square on a full-width dial page so solar-event icons can sit at the container edge. Soft radial seams blend neighboring rings; hovering the rings — or having a selected ring — animates `--clock-feather` down to sharpen them. A lighter elevation glow sits behind the rings; a separate conic wash along sunrise/sunset rays paints the sundown→sunrise horizon shadow **behind** the planet and spans the **full panel width** (sidebar overlaps it; the dial still shifts left via the gutter). Elevation still wraps the rim (angle = time, radius = elev): elev=0 is outside the planet so the path is larger than the rings; day extends toward the face edge; night keeps its elevation shape and is **shifted outward** so neither the path nor the large night sun overlaps the light rings. The sun is a solid white disc (black below the horizon) with a near glow and a larger blur shadow; the hour handle paints above so it is never covered. Path stroke is fixed-width: solid by day, brighter dashed by night, with dots at solar events and dashed spokes to those dots. Size is largest (2×) at sunrise/sunset and fixed through the night; it shrinks toward 1× at zenith and grows again toward sunset. Drag the sun (or handle) to scrub with magnetic snaps to solar events and rubber-band resistance leaving them; after release the time is sticky until reset (click the sun / restore control), an event is selected, the preview date changes, or the user leaves the editor. While sticky, a gold stroke on the outer tick radius runs from wall-clock now to the scrubbed time with a gold wash toward the center. Selecting a solar event pins the sun to that event. Idle without sticky/event is wall-clock now on the preview date. The linear sun chart is hidden in dial view. Solar-event icons open the event sidebar. Suggested lights stay in the legend. Remember the view in `localStorage` per HA user (`scene_extrapolation.lightView.v1.<user>`).
