...
- **Superseded in part:** 2026-08-29 — above the wheel, a brightness graph shows one point per assigned solar event (X = event time, Y = brightness). The area under the curve is filled with that lamp’s color at each event. Dragging a member point updates the draft brightness for the matching native scene (linked events share one draft). Events where this lamp is not yet in the scene show a `+` at brightness 0 (not draggable); clicking `+` adds the lamp (typical peers / event defaults) and then allows dragging. Clicking a member point focuses that scene like a wheel dot.
