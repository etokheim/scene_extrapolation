...
- **Superseded in part:** 2026-08-30 — light-ring glow is two non-interactive dial clones (scales 2.76 + 1.38, opacity 0.815) in a face-level layer above the horizon wash; **one** `blur(28px)` on that host (not per clone), with `translateZ(0)` compositor layers on bloom / rings / horizon / overlays so scrub does not re-rasterize static filters; planet shadow is `box-shadow` (not `filter: drop-shadow`); sun halo/shadow use SVG gradients (no CSS blur on moving discs). Interactive rings still stack above the hour handle. Event spokes match the night sun-path dash (`#d8e0ff`, `5 4`, 1px) at 50% opacity; night path is also 50%.
