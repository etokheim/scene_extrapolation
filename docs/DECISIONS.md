...
- **Superseded in part:** 2026-09-01 — the list-top control includes a right-aligned `ha-switch` matching on/off. Scene rows use a single bordered “table” surface (header + dividers) styled like HA’s config lists. Do **not** embed `ha-data-table` / `hass-tabs-subpage-data-table` — those stay in lazy HA frontend chunks with Lit column templates and are unreliable from this custom panel (same class of problem as automation sidebar elements).
