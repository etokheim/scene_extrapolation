...
- **Superseded in part:** 2026-08-30 — dial ring hover/selected use a stronger black `drop-shadow` and a white `::after` rim at 10% opacity (no scale grow); rim width is a fixed `1px` (`--ring-rim-w`) painted just inside the band edges so the outermost ring is not clipped at 100%. Sibling rings dim to 50% over a surface-colored disc (`--primary-background-color`) so they don’t mix with horizon/bloom (dim selector is `:not(.hovered):not(.selected)` so the active band stays fully opaque — a blanket `.clock-ring { opacity: 0.5 }` outranked the active `opacity: 1`). Only one band is highlighted at a time: while `.hovered` is set on another ring, `.selected:not(.hovered)` yields the rim/shadow and dims like siblings until the pointer leaves the planet. Clicking outside the rings (and outside legend/event/sidebar chrome) closes the light sidebar and clears selection. Soft mode keeps `--clock-feather` plus `--ring-soft-expand` (~1.35× feather) so opaque cores overlap and the surface never shows through seams; sharp mode (hover/selected host) zeros both so bands sit edge-to-edge. Hovering a ring shows its friendly name in a centered pill on the planet.
...
- **Superseded in part:** 2026-08-30 — dial chrome order (outer → inner): hour numbers on the face edge, then solar-event buttons, then the inset dial core (ticks/path/planet). Chrome inset grows if needed so event buttons stay outside the core.
