...
### Changed
- Dial view is a Solar Dial: watch face capped at 80vh with event buttons outside it and dashed spokes to the sun path, `24` at midnight, 10px/14px hour labels, 5-minute non-scaling ticks, a time handle that gaps through the sun’s white outline, outlined sun (15% smaller; day fill masked by the sunrise/sunset horizon as it crosses, black disc masked in on the night side; +10% size at the horizon), page-bleed sky (elevation conic with sunset→sunrise 50% darker + night/dusk shading + one sun-tinted horizon wash) feathered to the surface on all sides with one mask on the sky and solar events (not stacked ramps), with the timeline, readout, and draft banner overlaid, and full-day light rings (no center hole) in a floating circle opposite the handle
