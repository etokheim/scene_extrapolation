...
### Changed
- Dial view: ~100vh full-width canvas with solar events at the edge; sun path elev=0 sits outside the light-ring planet; sun rides the dashed path (no inset track); fill clipped to the sunrise→sunset day wedge (black disc underneath); size 2× from sunset→sunrise, easing to 1× at zenith; darker night wedges behind the planet spanning the full panel (sidebar overlaps; dial still shifts for the gutter); sticky scrub shows a gold rim arc from now to the override with an inward gold glow; drag the sun or hour handle to scrub (sticky); canvas touch-scrolls except on the sun/handle; fixed hour labels (HTML 10px/14px every 2h); 7.5-minute ticks with non-scaling 2px/1px strokes (majors at 6h); sun ~⅓ prior base size
