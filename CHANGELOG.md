...
### Changed
- Dial view is a Solar Dial: watch face capped at 80vh with event buttons outside it and dashed spokes to the sun path, `24` at midnight, 10px/14px hour labels, 5-minute non-scaling ticks, a time handle that gaps through the sun’s white outline, outlined sun (15% smaller; day fill masked by the sunrise/sunset horizon as it crosses, dark-blue disc masked in on the night side; +10% size at the horizon), page-bleed sky that follows the previewed time inside the masked disc (dark blue at night, not black; night/dusk shading + one sun-tinted horizon wash) with a vignette mask on the sky and solar events that floors at 25% opacity (not stacked ramps; the HA page chrome stays the theme surface); below-horizon event chips use the same dark blue, with the timeline, readout, and draft banner overlaid, and full-day light rings (no center hole) in a floating circle opposite the handle
