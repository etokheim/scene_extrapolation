...
### Changed
- Dial view: ~100vh full-width canvas with solar events at the edge; sun path elev=0 sits outside the light-ring planet; sun fill is clipped to the sunrise→sunset day wedge (black disc underneath) with a glow ramp along that horizon behind the planet (bleeds past the face); drag the sun or hour handle to scrub (sticky); canvas touch-scrolls except on the sun/handle; fixed hour labels (HTML 10px/14px every 2h); 7.5-minute ticks with non-scaling 2px/1px strokes (majors at 6h); sun ~⅓ prior size
