...
### Changed
- Dial view is a Solar Dial: inset watch face with event buttons outside it and dashed spokes to the sun path, `24` at midnight, 10px/14px hour labels, 5-minute non-scaling ticks, a time handle that gaps through the sun’s white outline, outlined sun (fill masked by the sunrise/sunset horizon as it crosses; +10% size at the horizon), full-container sky (elevation conic + night/dusk shading) with one sun-tinted horizon wash, and full-day light rings (no center hole) in a floating circle opposite the handle
