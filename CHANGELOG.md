...
### Changed
- Dial view is a Solar Dial: face-filling 24h scale (`24` at midnight) with ticks and labels inside a 10%-inset sun path, event icons on the path, radial time handle, draggable sun (sticky preview; drag clears an open event pin), solar-event sky wedges (night sunset→sunrise, darker dusk→dawn), and full-day light rings in a floating circle opposite the handle
