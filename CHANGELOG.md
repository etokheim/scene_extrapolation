...
### Changed
- Dial view: ~100vh full-width canvas with solar events in a wider chrome band outside a smaller dial core; sun path elev=0 sits outside the light-ring planet; smooth elev→radius oval (no night offset-blend warp); constant 1px non-scaling day/night path with 3px plain event dots + spokes; sun is day-wedge-clipped SVG fill with HTML outline/glow/shadow (outlined below horizon); wider horizon ramp plus farther sky-color wash; size 2× from sunset→sunrise, easing to 1× at zenith; darker night wedges behind the planet spanning the full panel; sticky scrub gold rim arc; magnetic scrub rubber-bands in the pull direction then eases to the cursor; click the sun or readout restore to reset to now; fixed hour labels (HTML 10px/14px every 2h); 7.5-minute ticks with non-scaling 2px/1px strokes
