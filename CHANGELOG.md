...
### Changed
- Dial view: ~100vh full-width canvas with solar events in a wider chrome band outside a smaller dial core; sun path is a perfect circle between the light-ring planet and the dial-core edge (1px non-scaling day/night stroke, 3px plain event dots + spokes); sun is solid white with day-wedge-clipped fill + glow (outline only below horizon); sky wash uses `--card-background-color` with a wider horizon ramp; sticky scrub shows a white rim arc and a short low-opacity white–gold inward wash; magnetic scrub rubber-bands in the pull direction then eases to the cursor; click the sun or readout restore to reset to now; fixed hour labels (HTML 10px/14px every 2h); 7.5-minute ticks with non-scaling 2px/1px strokes
