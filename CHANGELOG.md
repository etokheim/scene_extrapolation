...
### Changed
- Dial view: ~86vh dial with face-scaled chrome and in-bounds event buttons; sun path circle scales with day's peak; solid white day sun with soft day SVG shadow + night HTML shadow (glow/fill day-clipped only); sky wash follows Solar-face blues with a muted peach horizon; sticky scrub persists across date/location; event pin / reset uses strong ease-out; drag with event sidebar open closes on release; labeled-hour major ticks at ~30%/45% opacity
