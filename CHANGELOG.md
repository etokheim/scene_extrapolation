...
- Editor page max-width is 1024px in table view and 1920px in dial view; landscape dial timeline pins to the absolute right with day/month above it (HA date field overlays the label); suggestion chips share the date row in table view and sit under the date in dial view; dial time/elevation readout is absolute top-left so it does not push the clock down; location and Table/Dial view live in the header
