...
### Changed
- Dial view is a Solar Dial: face-filling watch container with a sun path 25% smaller inside it (`24` at midnight; ticks and labels inside the container), event icons on the path, a time handle that gaps through the sun’s white outline, outlined sun (fill clips below the horizon; +10% size at the horizon), solar-event sky wedges and horizon glow, and full-day light rings (no center hole) in a larger floating circle opposite the handle
