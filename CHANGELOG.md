...
### Changed
- Dial view: ~86vh dial with face-scaled chrome and in-bounds event buttons; sun path circle scales with day's peak; solid white day sun with soft SVG shadow inside a stronger warm glow (day-clipped); elevation sky wash uses outer sky blues (not white mid) plus horizon ramp; sticky scrub persists across date/location; event pin / reset eases the sun; drag with event sidebar open closes on release; labeled-hour major ticks at ~30%/45% opacity
