...
### Changed
- Dial view: ~100vh full-width canvas with solar events in a wider chrome band outside a smaller dial core; sun path is a perfect circle whose radius scales with the day's peak vs annual max (summer outer / winter inner, padded clear of planet and face); 1px non-scaling day/night stroke with 3px plain event dots; sun is solid white with day-wedge-clipped fill + white glow (outline only below horizon); rings fill to the center; sky wash uses `--card-background-color` with a horizon ramp; sticky scrub shows a 1px white rim arc and a short white–gold wash; magnetic snap windows at 60% of prior; ticks/labels white at 60% opacity; click the sun or readout restore to reset to now
